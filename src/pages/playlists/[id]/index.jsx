...
export const CreateSong = () => {
  ...
  const fetchPlaylist = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/v5/playlists/${playlistId}`, {
        headers: {
  ...
  const fetchSongs = async () => {
    try {
      const songPromises = playlist.songs.map(songId =>
        axios.get(`http://localhost:3000/api/v5/songs/${songId}`, {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:3000/api/v5/songs`, songData, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        },
      ...
      if (img) {
        ...
        const imgUploadResponse = await axios.post(`http://localhost:3000/api/v5/songs/${songId}/upload-image`, imgFormData, {
          headers: {
      ...
      if (url) {
        ...
        const songUploadResponse = await axios.post(`http://localhost:3000/api/v5/songs/${songId}/upload`, songFormData, {
          headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${token}`,
          },
      ...
      const playlistResponse = await axios.put(`http://localhost:3000/api/v5/playlists/${playlistId}`, {
